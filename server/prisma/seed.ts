...
import { hashPin, pinLookupHash } from "../src/auth";
...
async function main() {
  ...
  await prisma.trainer.upsert({
    ...
    update: {},
    create: { username: trainerUsername, passwordHash: await bcrypt.hash(trainerPassword, 10) },
  });
  ...
  for (const c of clients) {
    ...
    console.log(`Klant aangemaakt: ${c.naam} — pincode ${c.pin}`);
  }
